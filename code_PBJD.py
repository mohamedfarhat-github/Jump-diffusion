...
df = pd.read_csv(r'') # path to the exchange rate data
...
num_points=15
epsilon_interval=np.linspace(0.1,0.7,num_points)
...
for i in range(num_points):
    ...
    min_var=minim(log_likelihood_MJ, initial_guess,args=(log_returns,delta_t),method='Powell')
...
lambda_opt,mu_opt,sigma_opt,mu_j_opt,sigma_j_opt=lambda_opt_p,mu_opt_p,np.abs(sigma_opt_p),mu_j_opt_p,np.abs(sigma_j_opt_p)
...
block_time=pd.read_csv("") #path to the block time data
